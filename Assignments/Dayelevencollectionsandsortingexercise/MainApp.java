...
//		List<Order> orderlist =new ArrayList<Order>();
		...
		System.out.println("Obtaining a list of products belongs to category 'Books' with price>100");
		
//		for condition 1
		List<Product> booksGreaterThan100 = new ArrayList<Product>();
		for(Product p : productList) {
			if(p.getProductCategory().equals("Books") && p.getProductPrice() > 100) {
				booksGreaterThan100.add(p);
			}
		}
		System.out.println("List of the products belongs to category \"Books\" with price > 100");
...
//		for condition 2
		...
		for(Order order: orderList) {
			...
			for(int i = 0; i < order.getProducts().size(); i++) {
				if (order.getProducts().get(i).getProductCategory().equals("Baby")) {
...
//		for condition 3
		...
		for(Product product: productList) {
			if(product.getProductCategory().equals("Toys")) {
				toyProducts.add(product);
				product.setProductPrice(product.getProductPrice()-(product.getProductPrice()*20)/100);
			}
		}
...
//		for condition 3
		...
		LocalDate fromDate = LocalDate.of(2021, 3, 01);
		LocalDate toDate = LocalDate.of(2021, 4, 01);
		for(Order order: orderList) {
			boolean condition = false;
			for(Customer customer : customerList) {
				if(order.getCustomerId() == customer.getCustomerId()&&customer.getCustomerTier() == 2) {
...
//		for condition 4
		...
		for(Product product: productList) {
			if(product.getProductCategory().equals("Books") && product.getProductPrice() < 500) {
